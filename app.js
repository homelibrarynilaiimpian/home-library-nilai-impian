...
function authorNames(book){const names=(book?.book_authors||[]).map(x=>x?.author?.name).filter(Boolean);return names.length?names.join(', '):'Penulis tidak direkod'}
function firstCopy(book){return (book?.copies||[]).find(c=>!c.archived_at)||book?.copies?.[0]||null}
function coverHTML(url,className='book-cover'){if(url)return `<img class="${className}" src="${esc(url)}" alt="" loading="lazy" onerror="this.outerHTML='<div class=&quot;${className} cover-placeholder&quot;>HL</div>'">`;return `<div class="${className} cover-placeholder">HL</div>`}
...
function showApp(){$('#auth-screen').classList.add('hidden');$('#app-shell').classList.remove('hidden');const hash=location.hash.replace('#','');navigate(['home','catalogue','add','activity','profile','archive'].includes(hash)?hash:'home',false)}
function navigate(name,updateHash=true){const valid=['home','catalogue','add','activity','profile','archive'];if(!valid.includes(name))name='home';$$('.view').forEach(v=>v.classList.add('hidden'));$(`#view-${name}`).classList.remove('hidden');const navName=name==='archive'?'profile':name;$$('.nav-btn').forEach(b=>b.classList.toggle('active',b.dataset.nav===navName));const titles={home:'Library',catalogue:'Katalog',add:'Tambah Buku',activity:'Aktiviti',profile:'Profile',archive:'Archive & Trash'};$('#page-title').textContent=titles[name];if(updateHash)history.replaceState(null,'',`#${name}`);window.scrollTo({top:0,behavior:'smooth'});if(name==='catalogue')renderCatalogue();if(name==='activity')loadActivity();if(name==='archive')loadArchive()}
...
async function findOrCreatePublisher(name){if(!name)return null;const {data:found,error}=await supabase.from('publishers').select('id,name').eq('name',name).is('archived_at',null).limit(1);if(error)throw error;if(found?.[0])return found[0].id;const {data,error:insertError}=await supabase.from('publishers').insert({name,source:'WEBSITE',created_by:state.user.id,updated_by:state.user.id}).select('id').single();if(insertError)throw insertError;return data.id}
async function findOrCreateAuthor(name){const {data:found,error}=await supabase.from('authors').select('id,name').eq('name',name).is('archived_at',null).limit(1);if(error)throw error;if(found?.[0])return found[0].id;const {data,error:insertError}=await supabase.from('authors').insert({name,source:'WEBSITE',created_by:state.user.id,updated_by:state.user.id}).select('id').single();if(insertError)throw insertError;return data.id}
async function findOrCreateShelf(code){if(!code)return null;const {data:found,error}=await supabase.from('shelves').select('id,code').eq('code',code).is('archived_at',null).limit(1);if(error)throw error;if(found?.[0])return found[0].id;const {data,error:insertError}=await supabase.from('shelves').insert({code,name:code,created_by:state.user.id,updated_by:state.user.id}).select('id').single();if(insertError)throw insertError;return data.id}
async function getPrimaryCollection(){const {data,error}=await supabase.from('collections').select('id').eq('name','Koleksi Peribadi Zamri & Nor Azzah').is('archived_at',null).limit(1);if(error)throw error;return data?.[0]?.id||null}

async function addBook(e){e.preventDefault();const form=e.currentTarget;const submit=form.querySelector('button[type="submit"]');submit.disabled=true;submit.textContent='Menyimpan…';try{const title=formValue(form,'title');const authorList=formValue(form,'authors').split(',').map(x=>x.trim()).filter(Boolean);const isbn13=formValue(form,'isbn13').replace(/[^0-9Xx]/g,'');const year=formValue(form,'year');const publisherName=formValue(form,'publisher');const shelfCode=formValue(form,'shelf');let book=null;if(isbn13){const {data,error}=await supabase.from('books').select('id').eq('isbn_13',isbn13).is('archived_at',null).limit(1);if(error)throw error;book=data?.[0]||null}if(!book){const publisher_id=await findOrCreatePublisher(publisherName);const {data,error}=await supabase.from('books').insert({title,isbn_13:isbn13||null,publication_year:year?Number(year):null,language:formValue(form,'language')||null,publisher_id,cover_url:formValue(form,'cover_url')||null,source:'WEBSITE',created_by:state.user.id,updated_by:state.user.id}).select('id').single();if(error)throw error;book=data;for(let i=0;i<authorList.length;i++){const author_id=await findOrCreateAuthor(authorList[i]);const {error:linkError}=await supabase.from('book_authors').insert({book_id:book.id,author_id,author_order:i+1,created_by:state.user.id,updated_by:state.user.id});if(linkError)throw linkError}}else{toast('ISBN sudah wujud — naskhah baru ditambah pada judul sedia ada.')}const [collection_id,shelf_id]=await Promise.all([getPrimaryCollection(),findOrCreateShelf(shelfCode)]);const {error:copyError}=await supabase.from('copies').insert({book_id:book.id,collection_id,shelf_id,accession_no:formValue(form,'accession')||null,owner_label:formValue(form,'owner_label')||null,acquisition_date:formValue(form,'acquisition_date')||null,purchase_price:formValue(form,'purchase_price')?Number(formValue(form,'purchase_price')):null,purchased_from:formValue(form,'purchased_from')||null,condition:formValue(form,'condition')||null,notes:formValue(form,'notes')||null,legacy_call_no:formValue(form,'callno')||null,status:'AVAILABLE',source:'WEBSITE',created_by:state.user.id,updated_by:state.user.id});if(copyError)throw copyError;form.reset();toast('Buku berjaya disimpan.');await Promise.all([loadDashboard(),loadCatalogue(),loadActivity()]);navigate('catalogue')}catch(err){console.error(err);toast(err?.message||'Tak dapat simpan buku.',true)}finally{submit.disabled=false;submit.textContent='Simpan Buku'}}

function openEdit(book,copy){$('#book-dialog').close();const f=$('#edit-book-form');f.elements.book_id.value=book.id;f.elements.copy_id.value=copy.id;f.elements.title.value=book.title||'';f.elements.year.value=book.publication_year||'';f.elements.isbn13.value=book.isbn_13||'';f.elements.publisher.value=book.publisher?.name||'';f.elements.accession.value=copy.accession_no||'';f.elements.status.value=copy.status==='ARCHIVED'?'AVAILABLE':copy.status;f.elements.shelf.value=copy.shelf?.code||copy.shelf?.name||'';f.elements.condition.value=copy.condition||'';f.elements.acquisition_date.value=copy.acquisition_date||'';f.elements.purchase_price.value=copy.purchase_price??'';f.elements.purchased_from.value=copy.purchased_from||'';f.elements.owner_label.value=copy.owner_label||'';f.elements.callno.value=copy.legacy_call_no||'';f.elements.cover_url.value=book.cover_url||'';f.elements.notes.value=copy.notes||'';$('#edit-dialog').showModal()}
